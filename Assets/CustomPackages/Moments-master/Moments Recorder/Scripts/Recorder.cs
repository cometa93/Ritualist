...
namespace Moments
{
	...
	{
	    private const KeyCode SaveGifButtonKey = KeyCode.F1;
	    private const KeyCode StartRecordingKey = KeyCode.F2;
	    private const KeyCode PauseRecordingKey = KeyCode.F3;
	    ...
	    private void Update()
	    {
	        if (Input.GetKey(SaveGifButtonKey))
	        {
                Save();
	            return;
	        }

	        if (Input.GetKey(PauseRecordingKey))
	        {
	            Pause();
	            return;
	        }

	        if (Input.GetKey(StartRecordingKey))
	        {
	            Record();
	        }
	    }
		...
		void Init()
		{
			...
			{
				#if UNITY_EDITOR
				SaveFolder = Application.dataPath+"/Gifs/"; // Defaults to the asset folder in the editor for faster access to the gif file
				#else
